...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb):
        ...
        if self.use_scale_shift_norm:
            ...
            scale, shift = th.chunk(emb_out, 2, dim=1)  #切分tensor和cat函数相反
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.use_checkpoint = use_checkpoint
        self.dtype = th.float32 #if use_fp16 else th.float32
        self.num_heads = num_heads
        self.num_head_channels = num_head_channels
        self.num_heads_upsample = num_heads_upsample

        time_embed_dim = model_channels * 4
        self.time_embed = nn.Sequential(   # 时间编码    128-----128*4
        ...
        ch = input_ch = int(channel_mult[0] * model_channels)  #ch = input_ch = 1*128
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(conv_nd(dims, in_channels, ch, 3, padding=1))]
        )  # 时间步长编码序列；Conv2D （5，128）
        self._feature_size = ch   # 128
        input_block_chans = [ch]
        ds = 1
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):  #循环两次
                layers = [
                    ResBlock(
                        ch,
                        time_embed_dim,
                    ...
                    )  # 128(输入),128*4=512(time编码),不丢包，输出：1*128；use_scale_shift_norm = True
                ]
        ...
        if high_way:
            features = 32
            self.hwm = Generic_UNet(3, features, 3, 4, anchor_out=True, upscale_logits=True)
        # self.log2x = nn.Sequential(
        #     nn.SiLU(),
        #     conv_nd(dims, 4, 4, 3, padding=1),
        # )
        #  self.dbb_reparam.bias.data = bias.to(torch.float16)
    ...
    #     x = self.log2x(x)
    #     return x
#         model(      x_data, x_t_c, rescaled_t, **model_kwargs)
    def forward(self, x_data, x_t_c, timesteps, y=None):
        """
        ...
        if self.num_classes is not None:
            assert y.shape == (x_t_c.shape[0],)
            emb = emb + self.label_emb(y)
    
        h = x_t_c.type(self.dtype)
        x_data = x_data.type(self.dtype)
        # x_log = log.type(self.dtype)
        # x_logL = self.LoginC(x_log)
        # new_tensor = th.stack(slices, dim=1)
        # c = h[:,:-1,...]
        # anch = self.highway_forward(x_data) 
        anch, cal, de = self.highway_forward(x_data)   # cal[8,1,256,256]
        for ind, module in enumerate(self.input_blocks):
            if len(emb.size()) > 2:
                emb = emb.squeeze()
            if ind == 0 :
                # h = h + x_logL
                # h = th.cat((h, x_data),dim=1)
                # h = th.cat((h, x_data + x_logL),dim=1)
                h = module(h, emb)
        ...
        for ind, module in enumerate(self.output_blocks):
            if ind == 2:
                ...
                h = module(h, emb)
                # h = h + th.cat((de[0], de[0]),1).detach()
            elif ind == 10:
                h = th.cat([h, hs.pop()], dim=1)
                h = module(h, emb)
                h = h + th.cat((de[2], de[2], de[2], de[2]),1).detach()  
            elif ind == 6:
                h = th.cat([h, hs.pop()], dim=1)
                h = module(h, emb)
                h = h + th.cat((de[0], de[0]),1).detach()
...
class StackedConvLayers(nn.Module):
    def __init__(self, input_feature_channels, output_feature_channels, num_convs,
        ...
        self.blocks = nn.Sequential(   #ConvDropoutNormNonlin
...
class FFParser(nn.Module):
    def __init__(self, dim, h=128, w=65):
        super().__init__()
        self.complex_weight = nn.Parameter(th.randn(dim, h, w, 2, dtype=th.float32) * 0.02)
        self.w = w
        self.h = h
    ...
    def forward(self, x, spatial_size=None):
        ...
        # x = x.view(B, a, b, C)
        x = x.to(th.float32)
        x = th.fft.rfft2(x, dim=(2, 3), norm='ortho')
        weight = th.view_as_complex(self.complex_weight)
        x = x * weight
        x = th.fft.irfft2(x, s=(H, W), dim=(2, 3), norm='ortho')
...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if use_gaussian and num_tiles > 1:
            ...
            gaussian_importance_map = th.from_numpy(gaussian_importance_map)
        ...
        if all_in_gpu:
            ...
            if use_gaussian and num_tiles > 1:
                ...
                add_for_nb_of_preds = th.ones(patch_size, device=self.get_device())

            if verbose: print("initializing result array (on GPU)")
            aggregated_results = th.zeros([self.num_classes] + list(data.shape[1:]), dtype=th.half,
                                             device=self.get_device())

            if verbose: print("moving data to GPU")
            data = th.from_numpy(data).cuda(self.get_device(), non_blocking=True)

            if verbose: print("initializing result_numsamples (on GPU)")
            aggregated_nb_of_predictions = th.zeros([self.num_classes] + list(data.shape[1:]), dtype=th.half,
                                                       device=self.get_device())
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, th.tensor], mirror_axes: tuple,
                                           do_mirroring: bool = True,
                                           mult: np.ndarray or th.tensor = None) -> th.tensor:
        ...
        x = maybe_to_torch(x)
        result_torch = th.zeros([1, self.num_classes] + list(x.shape[2:]),
                                   dtype=th.float)
        ...
        for m in range(mirror_idx):
            ...
            if m == 1 and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(self(th.flip(x, (4, ))))
                result_torch += 1 / num_results * th.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(th.flip(x, (3, ))))
                result_torch += 1 / num_results * th.flip(pred, (3,))

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(th.flip(x, (4, 3))))
                result_torch += 1 / num_results * th.flip(pred, (4, 3))

            if m == 4 and (0 in mirror_axes):
                pred = self.inference_apply_nonlin(self(th.flip(x, (2, ))))
                result_torch += 1 / num_results * th.flip(pred, (2,))

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(self(th.flip(x, (4, 2))))
                result_torch += 1 / num_results * th.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(th.flip(x, (3, 2))))
                result_torch += 1 / num_results * th.flip(pred, (3, 2))

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(self(th.flip(x, (4, 3, 2))))
                result_torch += 1 / num_results * th.flip(pred, (4, 3, 2))
    ...
    def _internal_maybe_mirror_and_pred_2D(self, x: Union[np.ndarray, th.tensor], mirror_axes: tuple,
                                           do_mirroring: bool = True,
                                           mult: np.ndarray or th.tensor = None) -> th.tensor:
        # if cuda available:
        #   everything in here takes place on the GPU. If x and mult are not yet on GPU this will be taken care of here
        ...
        x = maybe_to_torch(x)
        result_torch = th.zeros([x.shape[0], self.num_classes] + list(x.shape[2:]), dtype=th.float)
    ...
    def forward(self, x, hs = None):
        ...
        if self.anchor_out:
            return tuple([i(j) for i, j in
                                        zip(list(self.upscale_logits_ops)[::-1], anch_outputs[:-1][::-1])]),seg_outputs[-1],de_outputs
