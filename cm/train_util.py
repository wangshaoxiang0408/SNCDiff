...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        while not self.lr_anneal_steps or self.step < self.lr_anneal_steps:
            try:
                    batch, cond, log, name = next(data_iter)
            ...
            self.run_step(batch, cond, log)   # [8,256,256]  [8,1,256,256]
    ...
    def run_step(self, batch, cond, log):
        batch=th.cat((batch, cond), dim=1)  #batch 包含标签
        cond={}    
        sample = self.forward_backward(batch, cond, log)
    ...
    def forward_backward(self, batch, cond, log):
        self.mp_trainer.zero_grad()
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(dist_util.dev())   #包含标签，数据
            micro_cond = {
                k: v[i : i + self.microbatch].to(dist_util.dev())
                for k, v in cond.items()
            }
            last_batch = (i + self.microbatch) >= batch.shape[0]
            t, weights = self.schedule_sampler.sample(micro.shape[0], dist_util.dev())  # t=8 weight=8
...
class CMTrainLoop(TrainLoop):
    def __init__(
        ...
        **kwargs,
        ...
        if target_model:
            ...
            self.target_model_param_groups_and_shapes =self.target_model_master_params #get_param_groups_and_shapes(
    ...
    def run_loop(self):
        ...
        while not self.lr_anneal_steps or self.step < self.lr_anneal_steps:
            ...
            self.run_step(batch, self.dyed)   # [8,256,256]  [8,1,256,256]
